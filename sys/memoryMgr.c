...
uint64_t* getPageEntry(int val, int type){
    uint64_t *t = NULL;
    switch(type){
        case 1:
            t = (uint64_t*) (0xFFFFFF7FBFDFE000UL | val);
            break;
        case 2:
            t = (uint64_t*) (0xFFFFFF7FBFC00000UL | val);
            break;
        case 3:
            t = (uint64_t*) (0xFFFFFF7F80000000UL | val);
            break;
        default:
            t = (uint64_t*) (0xFFFFFF0000000000UL | val);
            break;
    }
    if(*t&PAGE_P) return t;
    return NULL;
}
...
void freePages(){
    ...
    for(i = 0; i < 510; i++) {
        pml4 = getPageEntry(i<<3, 1);
        if (pml4){
            for (j = 0; j < 512; j++) {
                pdpe = getPageEntry(i << 12 | j << 3, 2);
                if (pdpe){
                    for (k = 0; k < 512; k++) {
                        pde =getPageEntry(i << 21 | j << 12 | k << 3, 3);
                        if (pde){
                            for (l = 0; l < 512; l++) {
                                pte = getPageEntry(i << 30 | j << 21 | k << 12 | l << 3, 4);
                                if(pte) freeFrame(pte);
                            }
                            freeFrame(pde);
                        }
                    }
